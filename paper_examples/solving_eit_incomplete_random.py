...
# masked data
mask = (np.random.rand(*dtn_data.shape)\
        < (num_non_zeros/np.prod(dtn_data.shape))).astype(np.int64)
...
plt.spy(mask)
plt.savefig("incomplete_random_fully_reconstruction_mask",
...
plt.colorbar()
# show
plt.savefig("incomplete_random_fully_reconstruction",
            bbox_inches='tight')   # save the figure to file
# plt.show()
...
dict_to_save = {'sigma': sigma_guess,  # sigma defined in each triangle
                ...
                'mask': mask,
                't': t, 'p': p}
sio.savemat("incomp_rand_full.mat",dict_to_save)